<script setup>

const pokemon = defineProps(["name", "xp", "height", "img", "loading"])

</script>
<template>
    <div class="card CardPokemonSelected " 
    :class="loading ? '' : 'animate__animated animate__fadeInDown'"
    >
        <img v-if="pokemon.name" :src="pokemon.img" height="300" class="card-img-top pt-2" :alt="pokemon.name" />
        <img v-else src="../assets/egg_pokemon.svg" height="300" class="card-img-top pt-2" alt="????" />
        <div class="card-body text-black ">
            <h5 class="card-title text-center">{{ pokemon.name || '???' }}</h5>
            <hr>
            <div class="row " style="padding-left: 20%;">
                <section class="col ">
                    <strong>XP: </strong>
                    <span>{{ pokemon.xp }}</span>
                </section>
                <section class="col">
                    <strong>Altura: </strong>
                    <span>{{ pokemon.height }} cm</span>
                </section>
            </div>
        </div>
    </div>
    
</template>


<style scoped>
.CardPokemonSelected {
    height: 75vh;
    background: rgb(191, 0, 255);
    background: radial-gradient(circle, rgb(191, 0, 255) 0%, rgba(19, 18, 70, 1) 75%);
}


</style>